...
const readForeignUntaggedVarian = {
  readUntaggedVariantImpl: v => v1 => Control$dMonad$dExcept$dTrans.monadThrowExceptT(Data$dIdentity.monadIdentity).throwError(Data$dNonEmpty.$NonEmpty(
    Foreign.$ForeignError("ForeignError", "Unable to match any variant member."),
    Data$dList$dTypes.Nil
  ))
};
const readForeignTaggedVariantN = {
  readVariantImpl: __ => v => Control$dMonad$dExcept$dTrans.monadThrowExceptT(Data$dIdentity.monadIdentity).throwError(Data$dNonEmpty.$NonEmpty(
    Foreign.$ForeignError("ForeignError", "Unable to match any variant member."),
    Data$dList$dTypes.Nil
  ))
};
...
const writeForeignTaggedVariant2 = dictIsSymbol => dictWriteForeign => () => dictWriteForeignTaggedVariant => (
  {
    writeVariantImpl: typeTag => valueTag => v => variant => {
      ...
      const $0 = dictWriteForeignTaggedVariant.writeVariantImpl(typeTag)(valueTag)(Type$dProxy.Proxy);
...
const writeForeignUntaggedVaria2 = dictIsSymbol => dictWriteForeign => () => dictWriteForeignUntaggedVariant => (
  {
    writeUntaggedVariantImpl: v => variant => {
      const $0 = dictWriteForeignUntaggedVariant.writeUntaggedVariantImpl(Type$dProxy.Proxy);
  ...
  return () => dictReadForeignTaggedVariant => (
    {
      readVariantImpl: v => o => {
        ...
        const name = dictIsSymbol.reflectSymbol(Type$dProxy.Proxy);
        return alt(dictReadForeignTaggedVariant.readVariantImpl(Type$dProxy.Proxy)(o))(bind(bind(readProp(dictIsSymbol1.reflectSymbol(Type$dProxy.Proxy))(o))(Foreign.readString(Data$dIdentity.monadIdentity)))(type_ => {
          if (type_ === name) { return bind(bind(readProp(valueTag)(o))(readImpl1))(v1 => pure({type: dictIsSymbol.reflectSymbol(Type$dProxy.Proxy), value: v1})); }
          return Control$dMonad$dExcept$dTrans.monadThrowExceptT(Data$dIdentity.monadIdentity).throwError(Data$dNonEmpty.$NonEmpty(
            Foreign.$ForeignError("ForeignError", "Did not match variant tag " + name),
            Data$dList$dTypes.Nil
          ));
        }));
      }
